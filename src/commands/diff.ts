...
import ora from 'ora';
import { getGitStatus, getGitDiff, isGitRepository, formatGitStatus } from '../git/operations';
...
export async function showGitDiff() {
  ...
  try {
    ...
    try {
      ...
      diffSpinner.fail(chalk.red(`❌ 获取改动失败: ${error instanceof Error ? error.message : '未知错误'}`));
    }
    
  } catch (error) {
    spinner.fail(chalk.red(`❌ 检查 Git 状态失败: ${error instanceof Error ? error.message : '未知错误'}`));
  }
}